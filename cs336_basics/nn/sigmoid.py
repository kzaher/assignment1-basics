...
class _SigmoidClampGrad(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, mu, s, min_grad, max_grad, beta, mode_flag):
        """
    ...
    def backward(ctx, grad_out):
        ...
        elif mode_flag == 3:  # smooth floor via softplus (differentiable knee)
            # abs_mod ≈ max(abs_g, gmin) but smooth around gmin
            grad = min_grad + F.softplus(grad - min_grad, beta=beta)
        else:
            raise Exception("unkown mode flag")
...
class SigmoidCustomGrad(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        mode_flag = {"default": 0, "one": 1, "max": 2, "smooth": 3}[self.gradient_mode]
